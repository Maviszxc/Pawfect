...
export default function AdminUsersPage() {
  ...
  // Filter users by archive status and active tab - UPDATED to exclude unverified users
  const filteredUsers = users.filter((user) => {
    // Filter out archived users
    if (user.isArchived) return false;

    // Filter out unverified users - ADDED THIS FILTER
  ...
  // Calculate counts for tabs - UPDATED to exclude unverified users
  const verifiedUsersCount = users.filter(u => u.verified && !u.isAdmin && !u.isArchived).length;
  const adminUsersCount = users.filter(u => u.isAdmin && !u.isArchived).length;
  const allUsersCount = users.filter(u => !u.isArchived && u.verified).length;
          ...
          <Card className="rounded-xl sm:rounded-2xl shadow-sm bg-white px-0 py-0 border-0">
            <CardContent className="p-4 sm:p-6 md:p-8">
              <div className="flex flex-col gap-4 sm:gap-6">
                ...
                {/* Tabs - UPDATED to remove Unverified tab */}
                <Tabs
                  value={activeTab}
